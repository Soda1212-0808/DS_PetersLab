...
for curr_animal=3:length(animals)
    ...
    recordings_all_training=plab.find_recordings(animal,[],'stim_wheel_right_stage2_mixed_VA_earphone');
    recordings_all_passive_visual=plab.find_recordings(animal,[],'lcr_passive');
    recordings_all_passive_audio=plab.find_recordings(animal,[],'hml_passive_audio_earphone');
    ...
    for curr_day=1:length(recordings_training)
        ...
        verbose=true
        ap.load_recording
        neural_num=length(template_tipdist);
        clearvars('-except',preload_vars{:});

groups = arrayfun(@(i)  i:min(i+30-1,neural_num)  ,1:30:neural_num,'UniformOutput',false);


        for curr_neuron=1:length(groups)
        
            plot_units=groups{curr_neuron};
        ...
        for workflow_idx = 1:3
            ...
            switch workflow_idx
                ...
                case 2
                    rec = plab.find_recordings(animal,rec_day,'lcr_passive');
                    rec_time = rec.recording{end};
                case 3
                    rec = plab.find_recordings(animal,rec_day,'hml_passive_audio_earphone');
                    rec_time = rec.recording{end};
            end
            verbose = true;
            ap.load_recording;
            ...
            for curr_modality = 1:2
                ...
                stim_vals = [90, -90, 0];
                temp_type=vertcat(trial_events.values(1:n_trials).TrialStimX);
                    use_align = arrayfun(@(x) stimOn_times( temp_type(1:length(stimOn_times))== x), ...
                ...
                elseif contains(bonsai_workflow,'hml') && curr_modality == 2
                    % use_align = stimOn_times([trial_events.values(1:n_trials).StimFrequence] == 8000);
                     stim_vals = [8000, 4000, 12000];
                                     temp_type=vertcat(trial_events.values(1:n_trials).StimFrequence);
        ...
        for curr_unit = 1:length(plot_units)
            ...
            title(nexttile(curr_h,tilenum(curr_h,3,2)),'Passive 1+');
             ...
             % ========== 保存图像 ==========
             save_folder = fullfile(Path, 'figures','unit_figures1', 'mPFC',animal,rec_day);
