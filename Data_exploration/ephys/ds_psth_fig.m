...
for workflow_idx = 1:3
    ...
    for curr_unit_idx = 1:length(plot_units)
        ...
        % Plot raster
        nexttile(h(curr_unit_idx),tilenum(h(curr_unit_idx),min(workflow_idx+1,4),1)); hold on;
...
for curr_unit = 1:length(plot_units)
    ...
    title(nexttile(curr_h,tilenum(curr_h,1,1)),'mixed');
    title(nexttile(curr_h,tilenum(curr_h,2,1)),'Task');
    title(nexttile(curr_h,tilenum(curr_h,3,1)),'V Passive');
     title(nexttile(curr_h,tilenum(curr_h,4,1)),'A Passive');

    legend(nexttile(curr_h,tilenum(curr_h,1,1)),{'Task','','V-Passive','','A-Passive'});
