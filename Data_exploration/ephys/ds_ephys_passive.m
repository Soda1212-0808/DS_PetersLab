...
anterior_learned_idx_VA_nA={[   ],  [   ],   [4],    [   ],  [   ],  [2 4],    [],    [],    [   ],  [   ],  [   ],  [   ],  [   ],  [   ],  [   ]};
...
for curr_animal=1:length(animals)
    ...
    for curr_day=1:4
        ...
        for curr_task=1:2
            ...
            for i=1:3
                baseline_bins = use_align{i}{1} + baseline_t;
                response_bins = use_align{i}{1} + response_t;
                ...
                event_response_p{curr_task}{i} =event_response_p_all(template_sort);

                % responsive_units{curr_task}{i} = find(event_response_p{curr_task}{i} < 0.05 | event_response_p{curr_task}{i} > 0.95);
                responsive_units{curr_task}{i} = find( event_response_p{curr_task}{i} > 0.95);
            end
        ...
        package_unit_psth_smooth_norm=cat(2,unit_psth_smooth_norm{:});
        ...
        % all_responsive_idx =(cell2mat(cellfun(@(x)   cell2mat(x),event_response_p,'UniformOutput',false))>0.95)+...
        %     -1*(cell2mat(cellfun(@(x)   cell2mat(x),event_response_p,'UniformOutput',false))<0.05);

        %
        % h1=nexttile
        % % 按照每一列逐步排序
        % imagesc(sortrows(all_responsive,[3 5 4  6 1 2], 'descend'));  % 按照第 col 列排序
        % xticks([1 2 3 4 5 6])
        % xticklabels(titles)
        % colormap(h1,ap.colormap('WK'));
...
save('C:\Users\dsong\Documents\MATLAB\Da_Song\Data_analysis\mice\process\processed_data_v2\ephys\mat_data\all_data.mat','animals','animals_all_event_response','animals_all_event_plot','animals_all_event_single_plot','-v7.3')
...
for curr_fig=[2 5 6 7]
    ...
    for i = 1:numGroups
        ...
        flat_data = cell2mat(Result2{i});
        all_data{i} = flat_data;
        means(i) = median(flat_data);
        errors(i) = std(flat_data) / sqrt(length(flat_data)); % 计算标准误差 SEM
    end
    ...
    for i = 1:numGroups
        if i<=numGroups/2
            b.CData(i, :) = barColors(1, :); % 设置每个柱子的颜色
        else
            b.CData(i, :) = barColors(2, :); % 设置每个柱子的颜色
        end
    end
    ...
    % 画散点图
    for i = 1:numGroups
        subCells = Result2{i}; % 取出当前组的子 cell
        numSubCells = length(subCells);

        for j = 1:numSubCells
            data_points = subCells{j}; % 取出子 cell 里的数值
            x_jittered = i + (rand(size(data_points)) - 0.5) * 0.2; % 轻微抖动避免重叠
...
for curr_fig=1:4
    ...
    for i = 1:numGroups
        ...
        flat_data = cell2mat(Result2{i});
        all_data{i} = flat_data;
        means(i) = mean(flat_data,'omitmissing');
        errors(i) = std(flat_data,'omitmissing') / sqrt(length(flat_data)); % 计算标准误差 SEM
    end
...
%%  heat map
used_idx=anterior_learned_idx_AnV_nV;
used_filter_idx=filtered_AnV_nV;
title_name='AnV nV';
...
single_neuron = cellfun(@(x) cat(1,x{:}),single_plot,'UniformOutput',false  );
single_neuron_2=cat(1,single_neuron{:});

response= cellfun(@(x) cat(1,x{:}),used_filter_idx,'UniformOutput',false  );
response_2=cat(1,response{:});



[~,max_idx_3]=max(single_neuron_2(: ,psth_use_t,3),[],2)
[~,max_idx_5]=max(single_neuron_2(: ,psth_use_t,5),[],2)

[~,sort_idx_3] = sortrows([response_2(:,3), max_idx_3],[1,2],["descend","ascend"]);
[~,sort_idx_5] = sortrows([response_2(:,5), max_idx_5],[1,2],["descend","ascend"]);
...
tiledlayout(2, 3); % 创建一个1行2列的布局
titles={'L','M','R','4k','8k','12k'};

used_neruons_3=single_neuron_2(sort_idx_3,:,:);
...
used_neruons_3_only_mean=permute(mean(used_neruons_3_sort(1:sum(response_2(:,3)),:,:),1,'omitmissing'),[2 3 1]);
used_neruons_3_only_error=permute(std(used_neruons_3_sort(1:sum(response_2(:,3)),:,:),0,1,'omitmissing')/sqrt(sum(response_2(:,3))),[2 3 1]);

nexttile
hold on
ap.errorfill(t_bins,used_neruons_3_only_mean(:,3),used_neruons_3_only_error(:,3),[0 0 1],0.1 ,0.5)
ap.errorfill(t_bins,used_neruons_3_only_mean(:,5),used_neruons_3_only_error(:,5),[1 0 0],0.1 ,0.5)


used_neruons_5=single_neuron_2(sort_idx_5,:,:);
...
clim(clim_value);
xlabel('time (s)')
yline(sum(response_2(:,5)),'Color',[0 1 0],'LineWidth',1)
end

used_neruons_5_only_mean=permute(mean(used_neruons_5_sort(1:sum(response_2(:,5)),:,:),1,'omitmissing'),[2 3 1]);
used_neruons_5_only_error=permute(std(used_neruons_5_sort(1:sum(response_2(:,5)),:,:),0,1,'omitmissing')/sqrt(sum(response_2(:,5))),[2 3 1]);
nexttile
hold on
...
single_plot_AV=cellfun(@(x,y)  x(y),animals_all_event_single_plot(~cellfun(@isempty, anterior_learned_idx_AV','UniformOutput',true)) ,...
    anterior_learned_idx_AV(~cellfun(@isempty, anterior_learned_idx_AV','UniformOutput',true))','UniformOutput',false);
AV_single_neuron = cellfun(@(x) cat(1,x{:}),single_plot_AV,'UniformOutput',false  );
AV_single_neuron_2=cat(1,AV_single_neuron{:});

AV_response= cellfun(@(x) cat(1,x{:}),filtered_AV,'UniformOutput',false  );
AV_response_2=cat(1,AV_response{:});

[~,AV_max_idx_3]=max(AV_single_neuron_2(: ,psth_use_t,3),[],2)
[~,AV_max_idx_5]=max(AV_single_neuron_2(: ,psth_use_t,5),[],2)

[~,AV_sort_idx_3] = sortrows([AV_response_2(:,3), AV_max_idx_3],[1,2],["descend","ascend"]);
[~,AV_sort_idx_5] = sortrows([AV_response_2(:,5), AV_max_idx_5],[1,2],["descend","ascend"]);

% [~,AV_sort_idx_3] = sortrows([AV_response_2(:,3), nanmean(AV_single_neuron_2(: ,psth_use_t,3),2)],[1,2],'descend');
% [~,AV_sort_idx_5] = sortrows([AV_response_2(:,5), nanmean(AV_single_neuron_2(: ,psth_use_t,5),2)],[1,2],'descend');

AV_use_idx= AV_response_2(:,3)==1|AV_response_2(:,5)==1;

% [~,AV_sort_idx_3] = sort(nanmean(AV_single_neuron_2(: ,psth_use_t,3),2),'descend');
% [~,AV_sort_idx_5] = sort(nanmean(AV_single_neuron_2(: ,psth_use_t,5),2),'descend');
figure('Position',[50 50 400 400]);
tiledlayout(2, 3); % 创建一个1行2列的布局

used_neruons_AV_3=AV_single_neuron_2(AV_sort_idx_3,:,:);
used_neruons_AV_3_sort=used_neruons_AV_3(AV_use_idx(AV_sort_idx_3),:,:);

for curr_idx=[3 5]
nexttile
imagesc(t_centers,[],used_neruons_AV_3_sort(:,:,curr_idx))
colormap(ap.colormap('BWR'));
clim(clim_value);
xlabel('time (s)')
title(titles{curr_idx})
yline(sum(AV_response_2(:,3)),'Color',[0 1 0],'LineWidth',1)
end
used_neruons_AV_3_only_mean=permute(mean(used_neruons_AV_3_sort(1:sum(AV_response_2(:,3)),:,:),1,'omitmissing'),[2 3 1]);
used_neruons_AV_3_only_error=permute(std(used_neruons_AV_3_sort(1:sum(AV_response_2(:,3)),:,:),0,1,'omitmissing')/sqrt(sum(AV_response_2(:,3))),[2 3 1]);
nexttile
hold on
ap.errorfill(t_bins,used_neruons_AV_3_only_mean(:,3),used_neruons_AV_3_only_error(:,3),[0 0 1],0.1 ,0.5)
ap.errorfill(t_bins,used_neruons_AV_3_only_mean(:,5),used_neruons_AV_3_only_error(:,5),[1 0 0],0.1 ,0.5)




used_neruons_AV_5=AV_single_neuron_2(AV_sort_idx_5,:,:);
used_neruons_AV_5_sort=used_neruons_AV_5(AV_use_idx(AV_sort_idx_5),:,:);

for curr_idx=[3 5]
nexttile
imagesc(t_centers,[],used_neruons_AV_5_sort(:,:,curr_idx))
colormap(ap.colormap('BWR'));
clim(clim_value);
xlabel('time (s)')
yline(sum(AV_response_2(:,5)),'Color',[0 1 0],'LineWidth',1)
end
used_neruons_AV_5_only_mean=permute(mean(used_neruons_AV_5_sort(1:sum(AV_response_2(:,5)),:,:),1,'omitmissing'),[2 3 1]);
used_neruons_AV_5_only_error=permute(std(used_neruons_AV_5_sort(1:sum(AV_response_2(:,5)),:,:),0,1,'omitmissing')/sqrt(sum(AV_response_2(:,5))),[2 3 1]);
nexttile
hold on
ap.errorfill(t_bins,used_neruons_AV_5_only_mean(:,3),used_neruons_AV_5_only_error(:,3),[0 0 1],0.1 ,0.5)
ap.errorfill(t_bins,used_neruons_AV_5_only_mean(:,5),used_neruons_AV_5_only_error(:,5),[1 0 0],0.1 ,0.5)

sgtitle('AV group')
savefig (gcf,[Path 'figures\summary\AV responsive neurons']);

 save('C:\Users\dsong\Documents\MATLAB\Da_Song\Data_analysis\mice\process\processed_data_v2\ephys\mat_data\buffered_data.mat','VA_single_neuron_2','VA_response_idx','AV_single_neuron_2','AV_response_idx','-v7.3')
...
for curr_animal=1:length(animals)
    ...
    for curr_probe=1:length(animals_probe_positions{curr_animal})
        ...
        non_responsive_neurons=~(...
            (animals_all_event_response{curr_animal}{curr_probe}{1} > 0.95) | ...
            (animals_all_event_response{curr_animal}{curr_probe}{1} < 0.05) | ...
            (animals_all_event_response{curr_animal}{curr_probe}{2} > 0.95) | ...
            (animals_all_event_response{curr_animal}{curr_probe}{2} < 0.05) | ...
            ((animals_all_event_response{curr_animal}{curr_probe}{2} > 0.95) & (animals_all_event_response{curr_animal}{curr_probe}{1} > 0.95)) | ...
            ((animals_all_event_response{curr_animal}{curr_probe}{1} < 0.05) & (animals_all_event_response{curr_animal}{curr_probe}{2} < 0.05)));

        % colorMap = ([0, 0, 1] .* (animals_all_event_response_p{curr_animal}{curr_probe}{1} > 0.95)) + ...
        %     ([0.5, 0.5, 1] .* (animals_all_event_response_p{curr_animal}{curr_probe}{1} < 0.05)) + ...
        %     ([1, 0, 0] .* (animals_all_event_response_p{curr_animal}{curr_probe}{2} > 0.95)) + ...
        %     ([1, 0.5, 0.5] .* (animals_all_event_response_p{curr_animal}{curr_probe}{2} < 0.05)) + ...
        %     ([0, 1, 0] .* ((animals_all_event_response_p{curr_animal}{curr_probe}{2} > 0.95) & (animals_all_event_response_p{curr_animal}{curr_probe}{1} > 0.95))) + ...
        %     ([0.5, 1, 0.5] .* ((animals_all_event_response_p{curr_animal}{curr_probe}{1} < 0.05) & (animals_all_event_response_p{curr_animal}{curr_probe}{2} < 0.05))) + ...
        %     ([0, 0, 0] .* non_responsive_neurons);

        colorMap =+([0,0,1].*(animals_all_event_response{curr_animal}{curr_probe}{1} > 0.95)) + ([0.5,0.5,1].*(animals_all_event_response{curr_animal}{curr_probe}{1} < 0.05))...
            +([1,0,0].*(animals_all_event_response{curr_animal}{curr_probe}{2} > 0.95))+([1,0.5,0.5].*(animals_all_event_response{curr_animal}{curr_probe}{2} <0.05))...
            +([0,1,0].*((animals_all_event_response{curr_animal}{curr_probe}{2} > 0.95)& (animals_all_event_response{curr_animal}{curr_probe}{1} > 0.95)))+([0.5,1,0.5].*((animals_all_event_response{curr_animal}{curr_probe}{1} <0.05)&(animals_all_event_response{curr_animal}{curr_probe}{2} <0.05)));


        scatter3(cells_position{curr_probe}(~non_responsive_neurons,1),cells_position{curr_probe}(~non_responsive_neurons,3),cells_position{curr_probe}(~non_responsive_neurons,2),10,colorMap(~non_responsive_neurons,:),'filled')
