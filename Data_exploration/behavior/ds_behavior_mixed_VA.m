...
tt = tiledlayout(f1,1,length(animals),'TileSpacing','tight');

f2=figure('Position',[50 50 length(animals)*300 900]);
tt2 = tiledlayout(f2,1,length(animals),'TileSpacing','tight');
for curr_animal_idx = 1:length(animals)
    ...
    success = nan(length(recordings),4);
    rxn_med = nan(length(recordings),4);
    stim2move_mad = nan(length(recordings),5);

    stim2move_mad_null = nan(length(recordings),5);

    frac_move_stimalign=cell(2,1);
    frac_move_stimalign{1} = nan(length(recordings),length(surround_time_points));
    ...
    for curr_recording =1: length(recordings)
        ...
        workflow_name{curr_recording}= recordings(curr_recording).workflow{index_real};
    ...
    t_animal = tiledlayout(tt,7,1,"TileSpacing","tight",'Padding','tight');
    t_animal.Layout.Tile =  curr_animal_idx;

    t_animal2 = tiledlayout(tt2,8,1,"TileSpacing","tight",'Padding','tight');
    t_animal2.Layout.Tile =  curr_animal_idx;
    ...
    idx_mixed_a= ~idx_left & ~idx_right &~idx_mixed_v;

    type=1*double(idx_left)+2*(idx_right)+3*double(idx_mixed_v)+4*double(idx_mixed_a);
    x = relative_day;
    yl = ylim;
    ...
    for curr_state=1:4
        ...
        ax=nexttile(t_animal2);
        plot(surround_time_points,wheel_vel_mean{curr_state}(1:end,:)','linewidth',2);
        xline(0,'color','k');
        ylabel('velocity');
        xlabel('Time from stim');
        set(ax,'ColorOrder',copper(length(recordings)));



        wheel_vel_max=  max(wheel_vel_mean{curr_state}(:,surround_time_points>0&surround_time_points<0.5),[],2);
        wheel_vel_min=      min(wheel_vel_mean{curr_state}(:,surround_time_points>0&surround_time_points<0.5),[],2);
        wheel_vel_max(wheel_vel_max==0)=nan;
        wheel_vel_min(wheel_vel_min==0)=nan;

        ax=nexttile(t_animal2);
        hold on
        plot(relative_day,wheel_vel_max);
        plot(relative_day,wheel_vel_min);
        plot(relative_day,wheel_vel_max, 'o', 'MarkerSize',2)
        plot(relative_day,wheel_vel_min, 'o', 'MarkerSize',2)
