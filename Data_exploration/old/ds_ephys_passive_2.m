...
for curr_animal=1 :length(animals)
            ...
            if curr_task==1||curr_task==2
                for curr_i=1:3
                    ...
                    % responsive_units{curr_task}{curr_i} = find( event_response_p{curr_task}{curr_i} > 0.95);
                     ...
                     % event_response_p{curr_task}{curr_i}( find(striatum_celltypes.tan))
                end
    ...
    save([Path 'single_mouse\' animal '_ephys.mat'],'all_cell_sorted',...
     ...
     'all_event_response_signle_neuron_h2','all_celltypes','all_cell_ccf_position_sorted',...
...
for curr_group=1:2
    ...
    for used_stim=[3 5 7 8]
        ...
        firing_rates=single_neuron_all_1(response_all(:,used_stim),:,used_stim);
        ...
        h = imagesc(ax1,y_edges(1:end-1), z_edges(1:end-1),map1 );  % 热图（注意转置）
        ...
        clim([0 1])
        colormap(ax1, ap.colormap('WR'));  % 设置热图 colormap
        % colorbar;
...
for curr_group=1:2
    ...
    for curr_stim=1:length(all_stim)
        ...
        neuron_coords= cellfun(@(x,y) x(y(:,used_stim),2),single_neuron_each_position,response_each_rec,'UniformOutput',false)
...
z_min = 250;
z_max = 600;
bin_size_z = 70; % 单位：μm，根据实际尺度调整
z_edges = z_min:bin_size_z:z_max;
z_edge_3=[250 350 450 600]


fig2 = figure('Position',[50 50 length(all_stim)*200 600]);
tl2 = tiledlayout(3,length(all_stim));
title(tl2, 'firing rates in AV overlay');
...
for curr_group=1:2
    ...
    for curr_stim=1:length(all_stim)
        ...
        % neuron_coords_all= cellfun(@(x)   x(:,2) ,single_neuron_each_position,'UniformOutput',false );
        neuron_coords_all= cellfun(@(x,y) x(y(:,used_stim)>0.99,2),single_neuron_each_position,response_each_rec,'UniformOutput',false)

        y=response_each_rec{1}
        if curr_stim<3
            temp_idx=[3 5]
        else
            temp_idx=[7 8]
        end
        neuron_coords= cellfun(@(x,y) x(y(:,temp_idx(1))>0.99&y(:,temp_idx(2))>0.99,2),single_neuron_each_position,response_each_rec,'UniformOutput',false)
        % x=single_neuron_each_position{1}
        % y=response_each_rec{1}

        firing_rates=cellfun(@(x,y) x(y(:,temp_idx(1))>0.99&y(:,temp_idx(2))>0.99,:,used_stim) ,single_neuron_each_rec_1,response_each_rec,'UniformOutput',false);
        ...
        firing_rates_bins =cellfun(@(x,idx) arrayfun(@(col) ...
            accumarray(idx, x(:,col), [length(z_edges)-1,1], @mean, NaN), ...
            1:size(x,2), 'UniformOutput', false),firing_rates,binIdx, 'UniformOutput', false);
        ...
        % plots in 3 groups
        firing_rates_all=cat(1,firing_rates{:});
        [~,~,binIdx_3] = histcounts(cat(1,projected_coords{:}), z_edge_3)
        firing_rates_bins_mean_3= arrayfun(@(col) ...
            accumarray(binIdx_3, firing_rates_all(:,col), [length(z_edge_3)-1,1], @mean, NaN), ...
            1:size(firing_rates_all,2), 'UniformOutput', false)
        sem_func = @(x) std(x,'omitmissing') / sqrt(numel(x));  % 定义计算SEM的函数
        ...
        figure(fig2)
        ax1=nexttile(tl2,4+curr_group*4+curr_stim-4)
        ap.errorfill(t_bins,cat(2,firing_rates_bins_mean_3{:})',...
            cat(2,firing_rates_bins_error_3{:})',parula(3),0.1,0.5)
        xlim([0 0.3])
...
% saveas(fig2,[Path 'figures\Figure\firing rates in AV overlay'  ], 'jpg');
