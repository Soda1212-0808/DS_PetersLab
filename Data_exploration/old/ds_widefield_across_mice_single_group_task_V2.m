...
data_type={'raw','kernels'}; % raw-1 kernels-2
t_t={t_task,t_kernels};
period={period_task,period_kernels}
use_t=t_t{type};
use_period=period{type}

align_time=2
align_time_name={'stim','move','iti move'}% stim-1 move-2  itimove-3;stage=[1 3]


colors{1}{1}=    [112  48 160]./255;
colors{1}{3}=    [212  148 160]./255;
colors{1}{2}=    [0 0 0];
colors{1}{4}=    [0.5 0.5 0.5];

colors{2}{1}=[  84 130 53 ]./255 ;
colors{2}{3}=[ 84 230 153 ]./255 ;
  colors{2}{2}=[0 0 0];
colors{2}{4}=[0.5 0.5 0.5];
...
t1 = tiledlayout(1, 2, 'TileSpacing', 'compact', 'Padding', 'compact');
    for curr_group=1:2
curr_data=all_data_cross_day{curr_group}{type,align_time};
buf1= cell2mat(cellfun(@(x) reshape(x,size(x,1)*size(x,2),size(x,3),size(x,4),size(x,5)),{curr_data},'UniformOutput',false)) ;
nexttile
  for curr_roi=[ 1 3];

      buf3_roi_l=  permute(mean(buf1(roi1(curr_roi).data.mask(:),:,:,:),1),[2,3,4,1]);
      buf3_roi_r= permute(mean(buf1(roi1(curr_roi+1).data.mask(:),:,:,:),1),[2,3,4,1]);
      % buf3_roi_peak=permute( max(buf3_roi_l(use_period,:,:)-buf3_roi_r(use_period,:,:),[],1)...
      %     ,[2,3,1]);
      
       buf3_roi_peak=permute( max(buf3_roi_l(use_period,:,:),[],1),[2,3,1]);

      buf3_roi_error=std(buf3_roi_peak,0,2,"omitmissing")./sqrt(size(buf3_roi_peak,2));
      buf3_roi_mean=nanmean(buf3_roi_peak,2);

    ap.errorfill(1:7,buf3_roi_mean(1:7),buf3_roi_error(1:7) ,colors{curr_group}{curr_roi},0.1,0.5);
    ap.errorfill(8:14,buf3_roi_mean(8:14),buf3_roi_error(8:14) ,colors{curr_group}{curr_roi},0.1,0.5);

xlim([1 14])
ylim(scale*[ 0 2])
xticks([3 7]); % 设置 y 轴的刻度位置（2代表naive stage中间位置，8代表stage1中间位置）
xticklabels({'Mod 1','Mod 2'}); % 设置对应的标签
ylabel('L-R \Delta F/F')

  end
legend({'','','','pl-mPFC','','','','al-mPFC'})
    end


% saveas(gcf,[Path 'figures\summary\figures\passive of symmetry'], 'jpg');
...
scale=0.0004
    for curr_image=[1 3 4 6]
        a1=nexttile
        imagesc(max(nanmean(all_data_image2{curr_group}{2,1}(:,:,period_kernels,1,:,curr_image),5),[],3))
        axis image off;
        ap.wf_draw('ccf', 'black');
        clim(scale .* [-1, 1]);
...
curr_group = select_group
    ...
    for curr_animal=1:length( data_all)
        ...
        buf3_roi=cell(length(roi1),1)
        buf3_roi_peak=cell(length(roi1),1)
        buf3_roi_stim=cell(length(roi1),1)
        for curr_roi= 1:length(roi1)
            buf3_roi{curr_roi}= cellfun(@(z) permute(mean(z(roi1(curr_roi).data.mask(:),:,:),1),[2,3,1]) , buf1, 'UniformOutput', false);
