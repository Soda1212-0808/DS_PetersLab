    ...
    for curr_stage=select_group
       ...
       main_preload_vars = who;
        ...
        stage1_pre_data =arrayfun(@(k) arrayfun(@(roi)  cellfun(@(x,y,z) x{roi}{k}(find(strcmp(y,z(find(cellfun(@(idx) strcmp(n1_name, idx),z,'UniformOutput',true)))),2,'first'),:)...
            ...
            (1:6)', 'UniformOutput', false),(1:3)', 'UniformOutput', false);
...
for curr_stage = select_group
    for workflow_idx = 1:2
        ...
        for img_idx = 1:5
            ...
            % 仅在第一行显示标题
            if curr_stage == 1 && workflow_idx == 1&& use_stim == 1
                title(titles{img_idx});
            end
            
            % 仅在后两行（curr_group == 2）的最后一列加 colorbar
            if curr_stage == 2 && img_idx == 5&& use_stim == 3
                colorbar;
            end
        end
        end
  ...
  saveas(gcf,[Path 'figures\summary\different_task_passive\ ' n1_name ' to ' n2_name ' imaging mpfc' strrep(workflow,'_',' ') ], 'jpg');
...
data_associated2_std2=cellfun(@(a) cellfun(@(x) cellfun(@(y) cellfun(@(z) std(cat(2,z{:}),0,2,"omitmissing")/sqrt(size(cat(2,z{:}),2)) ,y,'UniformOutput',false),...
  ...
  %%
  title1={'mmPFC','almPFC','PPC'}
title2={'VA visual','AV visual';'VA audio','AV aduio'}
title3={'stage1','stage2'}
colors={[ 1 0 0],[ 0.7 0.7 0.7],[ 1 0 0],[ 0.7 0.7 0.7],[ 1 0 0],[ 0.7 0.7 0.7]};

for curr_workflow = 1:2
    for curr_group = 1:2
        figure('Position',[50 50 800 800],'Name',[title2{curr_workflow, curr_group}]);
        ...
        for curr_stim = 1:3
            ...
            for curr_stage = 1:3 
                for curr_area = [1 2]
                    if mod(curr_area,2)~=0
                        nexttile;
                    end
                    hold on
                       if curr_stage == 1
                            ap.errorfill(1:3,n1_n2_mean_naive{curr_workflow}{curr_group}{curr_stim}{curr_area},...
                            n1_n2_error_naive{curr_workflow}{curr_group}{curr_stim}{curr_area} ,colors{curr_area},0.1,0.5);
                                                xlim([1 3]);

                    elseif curr_stage == 2
                        % plot(data_associated1_avg2{curr_workflow}{curr_group}{curr_stim}{curr_area});
                        % plot(data_associated1_avg2{i1}{i2}{i3}{i4}-data_associated1_avg2{i1}{i2}{i3}{i4+1});
                        ap.errorfill(1:7,data_associated1_avg2{curr_workflow}{curr_group}{curr_stim}{curr_area},...
                            data_associated1_std2{curr_workflow}{curr_group}{curr_stim}{curr_area} ,colors{curr_area},0.1,0.5);
                                            xlim([1 7]);

                    elseif curr_stage == 3
                        % plot(data_associated2_avg2{curr_workflow}{curr_group}{curr_stim}{curr_area});
                        % plot(data_associated2_avg2{i1}{i2}{i3}{i4}-data_associated2_avg2{i1}{i2}{i3}{i4+1});
                        if ~isempty(data_associated2_avg2{curr_workflow}{curr_group}{curr_stim}{curr_area})

                        
                        ap.errorfill(1:7,data_associated2_avg2{curr_workflow}{curr_group}{curr_stim}{curr_area},...
                            data_associated2_std2{curr_workflow}{curr_group}{curr_stim}{curr_area} ,colors{curr_area},0.1,0.5);
                        end
                                            xlim([1 7]);
                    ...
                    ylim(0.0002 * [0 1]);
                    xline(2.5);
                    if curr_stim == 1 &mod(curr_area,2)~=0&curr_stage==1
                        title('naive');
                    elseif curr_stim == 1 &mod(curr_area,2)~=0&curr_stage==2
                        title({'m-mPFC';'stage1'});
                           elseif curr_stim == 1 &mod(curr_area,2)~=0&curr_stage==3
                        title('stage2');
                    end
                end
            end

            for curr_stage = 1:3  % 先绘制 data_associated1_avg2，再绘制 data_associated2_avg2
                for curr_area = [3 4]
                    if mod(curr_area,2)~=0
                        nexttile;
                    end
                    ...
                    elseif curr_stage ==2
                        % plot(data_associated1_avg2{curr_workflow}{curr_group}{curr_stim}{curr_area});
                        % plot(data_associated1_avg2{i1}{i2}{i3}{i4}-data_associated1_avg2{i1}{i2}{i3}{i4+1});
                        ap.errorfill(01:7,data_associated1_avg2{curr_workflow}{curr_group}{curr_stim}{curr_area},...
                            data_associated1_std2{curr_workflow}{curr_group}{curr_stim}{curr_area} ,colors{curr_area},0.1,0.5);
                                            xlim([1 7]);
                    ...
                    elseif curr_stage ==3
                        ...
                        if ~isempty(data_associated2_avg2{curr_workflow}{curr_group}{curr_stim}{curr_area})
                            ...
                            ap.errorfill(01:7,data_associated2_avg2{curr_workflow}{curr_group}{curr_stim}{curr_area},...
                                data_associated2_std2{curr_workflow}{curr_group}{curr_stim}{curr_area} ,colors{curr_area},0.1,0.5);
                                                xlim([1 7]);
                    ...
                    sgtitle(title2{curr_workflow, curr_group} );

     saveas(gcf,[Path 'figures\summary\different_task_passive\' n1_name ' to ' n2_name ' associated plot mpfc' num2str(curr_group) strrep(all_workflow{curr_workflow},'_',' ') ], 'jpg');
...
%%
  ...
  data_associated1_avg=cellfun(@(a) cellfun(@(x) cellfun(@(y) cellfun(@(z) cellfun(@(z1) z1,...
      z,'UniformOutput',false) ,y,'UniformOutput',false),...
      x,'UniformOutput',false),a(select_group),'UniformOutput',false),data_associated1,'UniformOutput',false);

  data_associated1_avg2=cellfun(@(a) cellfun(@(x) cellfun(@(y) cellfun(@(z) nanmean(cat(3,z{:}),3) ,y,'UniformOutput',false),...
      x,'UniformOutput',false),a,'UniformOutput',false),data_associated1_avg,'UniformOutput',false);
  

 data_associated2_avg=cellfun(@(a) cellfun(@(x) cellfun(@(y) cellfun(@(z) cellfun(@(z1)  z1,...
      z,'UniformOutput',false) ,y,'UniformOutput',false),...
      x,'UniformOutput',false),a(select_group),'UniformOutput',false),data_associated2,'UniformOutput',false);

  data_associated2_avg2=cellfun(@(a) cellfun(@(x) cellfun(@(y) cellfun(@(z) nanmean(cat(3,z{:}),3) ,y,'UniformOutput',false),...
      x,'UniformOutput',false),a,'UniformOutput',false),data_associated2_avg,'UniformOutput',false);
 
  n1_n2_mean_naive=cellfun(@(a) cellfun(@(x) cellfun(@(y) cellfun(@(z) z(1:3,:) ,y,'UniformOutput',false),...
          ...
          for curr_stim = 1:3
              ...
              for curr_stage = 1:3
                  for curr_area = [1 ]
                      ...
                      if curr_stage == 1
                          imagesc(use_t,[],n1_n2_mean_naive{curr_workflow}{curr_group}{curr_stim}{curr_area} );
                      ...
                      elseif curr_stage == 2
                          % plot(data_associated1_avg2{curr_workflow}{curr_group}{curr_stim}{curr_area});
                          % plot(data_associated1_avg2{i1}{i2}{i3}{i4}-data_associated1_avg2{i1}{i2}{i3}{i4+1});
                          imagesc(use_t,[],data_associated1_avg2{curr_workflow}{curr_group}{curr_stim}{curr_area} );
                          % xlim([1 7]);
                          colormap( ap.colormap(['KW' Color{workflow_idx}]));
                          clim(0.0001 .* [-1, 1]);
                      ...
                      elseif curr_stage == 3
                          ...
                          if ~isempty(data_associated2_avg2{curr_workflow}{curr_group}{curr_stim}{curr_area})
                              ...
                              imagesc(use_t,[],data_associated2_avg2{curr_workflow}{curr_group}{curr_stim}{curr_area} );
                              colormap( ap.colormap(['KW' Color{workflow_idx}]));
                              clim(0.0001 .* [-1, 1]);
  ...
  for curr_group = select_group
      for workflow_idx = 1:2
          for use_stim=1:3
              ...
              a1=nexttile(t,[1,2])
              % nexttile
              imagesc(use_t,[ ], data_all{workflow_idx}{curr_group}{use_stim}{1})
              yline(3.5)
              yline(5.5)
              yline(5+5+0.5)
              yline(6+5+0.5)
              ...
              %
              n1_n2_mean_naive= mean(data_naive_peak{workflow_idx}{curr_group}{use_stim}{1},2,"omitnan") ;
              n1_n2_mean=mean(data_peak{workflow_idx}{curr_group}{use_stim}{1},2,"omitnan");
              
               % n1_n2_mean=mean(data_peak{workflow_idx}{2}{use_stim}{1},2,"omitnan")

              n1_n2_error_naive= std(data_naive_peak{workflow_idx}{curr_group}{use_stim}{1}',1,"omitnan")/sqrt(size(data_naive_peak{workflow_idx}{curr_group}{use_stim}{1},2)) ;
              n1_n2_error= std(data_peak{workflow_idx}{curr_group}{use_stim}{1}',1,"omitnan")/sqrt(size(data_peak{workflow_idx}{curr_group}{use_stim}{1},2));
  ...
  % saveas(gcf,[Path 'figures\summary\different_task_passive\ ' n1_name ' to ' n2_name ' plot mpfc' strrep(workflow,'_',' ') ], 'jpg');
...
for curr_stage = select_group
    for workflow_idx = 1:2
        ...
        for img_idx = 1:5
            ax = nexttile;
            if ~isempty(data_imaging_all{workflow_idx}{curr_stage}{img_idx})
            imagesc(data_imaging_all{workflow_idx}{curr_stage}{img_idx}(:,:,use_stim)-fliplr(data_imaging_all{workflow_idx}{curr_stage}{img_idx}(:,:,use_stim)));
            else imagesc(zeros(450,426));
            end
            axis image off;
            ap.wf_draw('ccf', 'black');
...
saveas(gcf,[Path 'figures\summary\different_task_passive\asymmtery imaging mpfc activity in ' n1_name ' to ' n2_name 'in ' strrep(workflow,'_',' ') ], 'jpg');
...
for curr_stage = select_group
 ...
 a1=nexttile(t,[1,2])
% nexttile
imagesc(use_t,[ ], data_all{workflow_idx}{curr_stage}{use_stim}-data_all_r{workflow_idx}{curr_stage}{use_stim})
yline(3.5)
yline(5.5)
...
n1_n2_mean_naive= mean(data_naive_peak{workflow_idx}{curr_stage}{use_stim}-data_naive_peak_r{workflow_idx}{curr_stage}{use_stim},2,"omitnan") ;
n1_n2_mean=mean(data_peak{workflow_idx}{curr_stage}{use_stim}-data_peak_r{workflow_idx}{curr_stage}{use_stim},2,"omitnan");

n1_n2_error_naive= std(data_naive_peak{workflow_idx}{curr_stage}{use_stim}'-data_naive_peak_r{workflow_idx}{curr_stage}{use_stim}',1,"omitnan")/...
    sqrt(size(data_naive_peak{workflow_idx}{curr_stage}{use_stim},2)) ;
n1_n2_error= std(data_peak{workflow_idx}{curr_stage}{use_stim}'-data_peak_r{workflow_idx}{curr_stage}{use_stim}',1,"omitnan")/sqrt(size(data_peak{workflow_idx}{curr_stage}{use_stim},2));
...
sgtitle({[n1_name ' to ' n2_name];['in ' strrep(workflow,'_',' ')]})
    end
end
end
saveas(gcf,[Path 'figures\summary\different_task_passive\asymmetry plot mpfc activity in ' n1_name ' to ' n2_name 'in ' strrep(workflow,'_',' ') ], 'jpg');
...
%%
figure('Position', [50 50 700 850]);
t = tiledlayout(8, 4, 'TileSpacing', 'tight', 'Padding', 'none');  
...
for curr_stage = select_group
 ...
 nexttile(t)
% nexttile
buffer_plot=data_all{workflow_idx}{curr_stage};
plot_mean=cellfun(@(x) cellfun(@(y) mean(y(17:19,:),1),x(ii),'UniformOutput',false),buffer_plot,'UniformOutput',true);
plot_error= cellfun(@(x) cellfun(@(y) std(y(17:19,:))/sqrt(3),x(ii),'UniformOutput',false),buffer_plot,'UniformOutput',true);
plot_peak_mean{(curr_stage-min(select_group))*2+workflow_idx}=cellfun(@(x) cellfun(@(y) mean(max(y(17:19,use_period),[],2),1),x(ii),'UniformOutput',true),buffer_plot,'UniformOutput',true);
plot_peak_error{(curr_stage-min(select_group))*2+workflow_idx}=cellfun(@(x) cellfun(@(y) std(max(y(17:19,use_period),[],2),1)/sqrt(3),x(ii),'UniformOutput',true),buffer_plot,'UniformOutput',true)';
...
%     title ('visual passive')
sgtitle({[n1_name ' to ' n2_name];['in ' strrep(workflow,'_',' ')]})
 saveas(gcf,[Path 'figures\summary\different_task_passive\ plot mpfc activity in ' n1_name ' to ' n2_name 'in mxied of' strrep(workflow,'_',' ') ], 'jpg');
...
for curr_stage = select_group
 ...
 nexttile(t)
% nexttile
buffer_plot=cellfun(@(x,y) x-y,data_all{workflow_idx}{curr_stage},data_all_r{workflow_idx}{curr_stage},'UniformOutput',false);
...
plot_peak_mean{(curr_stage-1)*2+workflow_idx}=cellfun(@(x) mean(max(x(17:19,use_period),[],2),1),buffer_plot,'UniformOutput',true);
plot_peak_error{(curr_stage-1)*2+workflow_idx}=cellfun(@(x) std(max(x(17:19,use_period),[],2),1)/sqrt(3),buffer_plot,'UniformOutput',true)';
...
saveas(gcf,[Path 'figures\summary\different_task_passive\non learner asymmetry plot mpfc activity in ' n1_name ' to ' n2_name 'in mxied of' strrep(workflow,'_',' ') ], 'jpg');
