...
for curr_animal=1:length(animals)
    ...
    for curr_idx=1:length(indx_A)
        ...
        buffer.data_A_task_react_index{curr_animal}{curr_idx}=raw_data_task.react_index(indx_A(curr_idx));
    ...
    for curr_idx=1:length(indx_M)
        ...
        buffer.data_M_task_react_index{curr_animal}{curr_idx}=raw_data_task.react_index(indx_M(curr_idx));
...
% 定义数据和标题的索引
if selected_group<=2
data_fields = {'basline_lcr', 'V_90', 'A_90', 'basline_hml', 'V_8k', 'A_8k'};
elseif selected_group>=3
    data_fields = {'basline_lcr', 'A_90', 'V_90', 'basline_hml', 'A_8k', 'V_8k'};
end
titles = {'stage 0 of visual passive stimulus', ...
          'stage 1 of visual passive stimulus', ...
          'stage 2 of visual passive stimulus', ...
          'stage 0 of auditory passive stimulus', ...
          'stage 1 of auditory passive stimulus', ...
          'stage 2 of auditory passive stimulus'};

% 处理每个图像的显示
for i = 1:6
    a = nexttile(i);

    data_var=evalin('base',['buffer.data_', data_fields{i}]);
    data_var_mean=[ 'mean_image_',data_fields{i}];

    % 根据字段获取对应的数据
    selected_data =data_var(find(animals_group == selected_group));
    
    % 对最后几帧进行平均
    if i == 1 || i == 4  % baseline 的情况
        last_data = cellfun(@(x) mean(cat(4, x{:}), 4), selected_data, 'UniformOutput', false);
       eval([data_var_mean ' = mean(cat(4, last_data{:}), 4);']);
    else
        data_b = cellfun(@(x) x(max(1, end-4):end), selected_data, 'UniformOutput', false);
        data_b1 = cellfun(@(x) mean(cat(4, x{:}), 4), data_b, 'UniformOutput', false);
        eval([data_var_mean ' = mean(cat(4, data_b1{:}), 4);']);

    end
  mean_image= eval(data_var_mean);
    % 显示图像
    buffer_image = max(mean_image(:,:,period_passive), [], 3);
    ...
    % 设置颜色范围和色彩映射
    if i <= 3  % visual
        colormap(a, ap.colormap('WB'));
    else  % auditory
        colormap(a, ap.colormap('WR'));
    end
    clim(scale_all .* [0, 1]);
    
    % 设置标题
    title(titles{i}, 'FontSize', 14, 'FontWeight', 'normal');
end

% 添加图像的总标题
sgtitle(['Passive stimulation in group ' num2str(selected_group) ' (0-' num2str(passive_boundary*1000) 'ms)']);

% 保存图像
saveas(gcf, fullfile(Path, 'figures', ['averaged mice in visual auditory pre post_' num2str(selected_group) '.jpg']), 'jpg');
savefig(gcf, fullfile(Path, 'figures', ['averaged mice in visual auditory pre post_' num2str(selected_group) '.fig']));
...
if selected_group >= 3
   image_data = {mean_image_basline_lcr(:,:,k), mean_image_A_90(:,:,k), mean_image_V_90(:,:,k), ...
              mean_image_basline_hml(:,:,k), mean_image_A_8k(:,:,k), mean_image_V_8k(:,:,k)};
else
    image_data = {mean_image_basline_lcr(:,:,k), mean_image_V_90(:,:,k), mean_image_A_90(:,:,k), ...
              mean_image_basline_hml(:,:,k), mean_image_V_8k(:,:,k), mean_image_A_8k(:,:,k)};
end
% 图像数组，包含所有图像
...
%% wide imaging of whole brain of each mice
for u= 1:length(animals)

animal=animals{u};
animals_group = [1 1 1 1 1 1 1 2 2 3 3 3 4 4 4 4 4];
selected_group = animals_group(u);
scale_all = 0.003;
figure('Position', [50 50 1500 900]);
...
% 添加图像的总标题
sgtitle(['Passive stimulation in ' animal ' (0-100ms)']);

% 保存图像
saveas(gcf, fullfile(Path, 'figures', ['single mice in visual auditory pre post_' animal '.jpg']), 'jpg');


% 做视频 每只小鼠
close all
% 创建一个 VideoWriter 对象，指定文件名和格式
videoFilename = ['figures\visual_auditory_passive_' animal '.avi'];
...
for u=1:2
    if u==1
        V_data_0 = buffer.data_V_task_s2m;
        V_reindx=buffer.data_V_task_react_index;
        A_data_0 = buffer.data_A_task_s2m;
        A_reindx=buffer.data_A_task_react_index;

    else
        V_data_0 = buffer.data_V_task_rxt;
        V_reindx=buffer.data_V_task_react_index;
        A_data_0 = buffer.data_A_task_rxt;
        A_reindx=buffer.data_A_task_react_index;
  ...
  % 对矩阵进行操作
    A_data = cellfun(@(x) cell2mat(x), A_data_0, 'UniformOutput', false);
    A_reindx=  cellfun(@(A) [zeros(1, find(cell2mat( A )> 0, 1)-1), ones(1, numel(A) - find(cell2mat( A) > 0, 1) + 1)],A_reindx, 'UniformOutput', false);
...
savefig(gcf,[Path 'figures\behaviors_plot_reaction time_VA_AV.fig'], 'compact');
