...
for curr_group=1:2
    for   workflow_idx =curr_group;
        ...
        main_preload_vars = who;
        figure('Position',[50 50 250 350])
        mainLayout = tiledlayout(1, 3, 'TileSpacing', 'tight', 'Padding', 'tight');
        leftLayout = tiledlayout(mainLayout, 3, 1, ...
        ...
        Color={'B','R'};
        % darw iamge
        buf_images_all1=permute(mean(max(data_all_video{curr_group}{2}{workflow_idx}(:,:,use_period,:,:,5),[],3),5,'omitnan'),[1 2 4 3]);
        buf3_roi_stim1_mean=cell(3,1);
            buf3_roi_stim1_error=cell(3,1);
        for curr_day=1:3
            ...
            % darw selectivity
            buf3_roi_peak=cell(size(data_all_video{curr_group}{2}{workflow_idx}(:,:,:,:,:,5),5),1);
            buf3_roi_stim=cell(size(data_all_video{curr_group}{2}{workflow_idx}(:,:,:,:,:,5),5),1);
            for curr_animal=1:size(data_all_video{curr_group}{2}{workflow_idx}(:,:,:,:,:,5),5)

                buf1=cellfun(@(z) reshape(z,size(z,1)*size(z,2),size(z,3),size(z,4)),...
                    {data_all_video{curr_group}{2}{workflow_idx}(:,:,:,:,curr_animal,5)}, 'UniformOutput', false);
                for curr_roi= 1:length(roi1)
                    buf3_roi{curr_roi}=permute(nanmean(buf1{1}(roi1(curr_roi).data.mask(:),:,:),1),[2,3,1]);
            ...
            buf3_roi_stim2=permute(cat(4,buf3_roi_stim1{:}),[2 3 4 1]);
            buf3_roi_stim1_mean{curr_day}=mean(buf3_roi_stim2,3,'omitnan');
            buf3_roi_stim1_error{curr_day}=std(buf3_roi_stim2,0,3,'omitmissing')/sqrt(size(buf3_roi_stim2,3));

            buf3_roi_peak1= cellfun(@(x) cat(3,x{:}), buf3_roi_peak,'UniformOutput',false);
            buf3_roi_peak2=permute(cat(4,buf3_roi_peak1{:}),[2,3,4,1]);
            buf3_roi_peak_mean{curr_group}=mean(buf3_roi_peak2,3,'omitnan');
            buf3_roi_peak_error{curr_group}=std(buf3_roi_peak2,0,3,'omitmissing')./sqrt(size(buf3_roi_peak2,3));
        ...
        for curr_side=1:2
            ...
            for curr_area=select_area
                nexttile(rightLayout)
                hold on
                  for curr_day=1:3
                ap.errorfill(use_t,buf3_roi_stim1_mean{curr_day}(:,curr_area+curr_side-1),...
                    buf3_roi_stim1_error{curr_day}(:,curr_area),line_colors{workflow_idx,curr_day},0.1,1,1)
                   xlim([-0.1 0.5])
                   if curr_area==7
                       ylim(0.0001*[-2 5])
                   else
                     ...
                     text(0.95, 0.9, roi1(curr_area+curr_side-1).name, ...
...
saveas(gcf,[Path 'figures\summary\figures\fig1 passive images V&A pre&well trianed ' ], 'jpg');


figure('Position',[50 50 300 150])
t = tiledlayout(1, 2, 'TileSpacing', 'tight', 'Padding', 'tight');
colors = { [0.5 0.5 1],[1 0.5 0.5]}; % 定义颜色
scale=0.0002;
...
for curr_area=[1 3]
    ...
    for curr_group=1:2
        switch curr_group
            case 1
                errorbar( 1:3 ,buf3_roi_peak_mean{curr_group}(:,curr_area),buf3_roi_peak_error{curr_group}(:,curr_area),'k','MarkerSize',20, 'LineWidth', 2,'Color',colors{curr_group})
            case 2
                errorbar( 1:3 ,buf3_roi_peak_mean{curr_group}([1 3 2],curr_area),buf3_roi_peak_error{curr_group}([1 3 2],curr_area),'k','MarkerSize',20, 'LineWidth', 2,'Color',colors{curr_group})
        end
    end
    hold on
        ...
        for curr_roi = [1  3  ]
            switch curr_roi
                case 1
                    ...
                    hold on
                case 2
                    nexttile(t,10*curr_group+5*curr_passive-18)
                    hold on
                case 3
                    nexttile(t,10*curr_group+5*curr_passive-10)
                    hold on
                case 4
                    nexttile(t,10*curr_group+5*curr_passive-16)
                    hold on
            end

            % curr_stim = 4-curr_passive

            for curr_passivestim=1:2
            % value_columns = [1 5 8 9];
            curr_value = buf3_roi_mean_crossday{curr_group}{curr_passive}{curr_roi}{curr_passivestim};
            curr_error = buf3_roi_error_crossday{curr_group}{curr_passive}{curr_roi}{curr_passivestim};
             color = colors{curr_passive}{curr_group, curr_passivestim};
            % errorbar(1:4, curr_value, curr_error, 'Color', color, 'CapSize', 6, 'LineStyle', '-','LineWidth',2);
            ap.errorfill(1:8,curr_value(4:11),curr_error(4:11),color,0.1 )
            ap.errorfill(9:13,curr_value(22:26),curr_error(22:26),color,0.1 )
            ...
            if curr_roi==3 & curr_passive==1
                legend({'','task cue','','','','','','other cue'},"Box","off")
            end
            ylim(scale*[0 1.2])
            ylabel('\Delta F/F_0')
...
% saveas(gcf,[Path 'figures\summary\figures\passive of 3 stage images and selectivity 2'], 'jpg');

%% permutation test

% save([ Path 'summary_data\passive kernels of images crossday corsstime.mat'],...
%     'buf3_roi_l_acrosstime','buf3_roi_l_error_crosstime','buf3_roi_l_mean_crosstime','buf3_roi_error_crossday',...
