...
kernels_v=permute(mean(max(all_data_image2{1}{2,1}(:,:,period_kernels,:,:,[1 4]),[],3),5),[1,2,6,5,4,3]);
kernels_a=permute(mean(max(all_data_image2{2}{2,1}(:,:,period_kernels,:,:,[1 4]),[],3),5),[1,2,6,5,4,3]);

curr_roi=1
temp_data=permute(all_data_image2{1}{2}(:,:,:,1,:,[1 4]),[1,2,3,5,6,4]);
temp_data_1=reshape(temp_data,size(temp_data,1)*size(temp_data,2),size(temp_data,3),size(temp_data,4),size(temp_data,5));
temp_data_2= permute(nanmean(temp_data_1(roi1(curr_roi).data.mask(:),:,:,:),1),[2,4,3,1]);
...
for curr_stage=1:2
a1=nexttile
imagesc(kernels_v(:,:,curr_stage))
axis image off
ap.wf_draw('ccf','black');
clim(0.0004.*[0,1]);
colormap(a1,ap.colormap(['W' colors{1}]));
axis image;
end

temp1=cellfun(@(x) cat(4,x{5:9}), all_data_cross_day{1}{2}{1},'UniformOutput',false);
temp2=cat(4,temp1{:});
kernels_v_each_day=permute(max(temp2(:,:,period_kernels,:),[],3),[1 2 4 3]);

temp1=cellfun(@(x) cat(4,x{5:9}), all_data_cross_day{2}{2}{1},'UniformOutput',false);
temp2=cat(4,temp1{:});
kernels_a_each_day=permute(max(temp2(:,:,period_kernels,:),[],3),[1 2 4 3]);



nexttile
ap.errorfill(use_t, mean(temp_data_2,3))
%% images across time aligned to stim 
Color={'B','R'};
group_name={'visual','auditory'}
row_labels={'pre learn','post learn'}
for curr_group=1
curr_video=permute(nanmean(all_data_image2{curr_group}{2}(:,:,:,1,:,[1 4]),5),[1,2,3,6,5,4]);
scale=0.0004;
time_period=[-0.05 0.3]
figure('Position', [50 50 110*length(find(use_t>=time_period(1)& use_t<=time_period(2))) 160*(size(curr_video,4))]);
...
for curr_group=1:2
curr_video=permute(nanmean(all_data_image2{curr_group}{2,3}(:,:,:,1,:,used_stage),5),[1,2,3,6,5,4]);
scale=0.0002;
...
temp_data_1=reshape(temp_data,size(temp_data,1)*size(temp_data,2),size(temp_data,3),size(temp_data,4),size(temp_data,5));
temp_data_2= permute(nanmean(temp_data_1(roi1(curr_roi).data.mask(:),:,:,:),1),[2,4,3,1]);
plot_mean=nanmean(temp_data_2,3);
...
for curr_group=select_group
    ...
    curr_area=[1 3];
    buf3_roi=cell(length( all_data_cross_day{curr_group}{2}{1}),1);
    buf3_roi_peak=cell(length( all_data_cross_day{curr_group}{2}{1}),1);
    buf3_roi_stim=cell(length( all_data_cross_day{curr_group}{2}{1}),1);
    for curr_animal=1:length( all_data_cross_day{curr_group}{2}{1})
...
A=all_buf_data_each{1}{overlays{curr_overlay}(1)};
B=all_buf_data_each{2}{overlays{curr_overlay}(2)};

p_vals{curr_overlay}=ds.
