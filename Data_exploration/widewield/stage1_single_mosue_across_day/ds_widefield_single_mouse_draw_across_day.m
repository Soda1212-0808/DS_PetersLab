...
ylim(0.002*[-1,3])
   title(raw_data_lcr1.workflow_day {i},[workflow_stage{raw_data_lcr1.workflow_type(i)+1} '-' learn_name{raw_data_lcr1.learned_day(i)+1} ])
    end
end
...
drawnow
saveas(gcf,[Path 'figures\summary\passive_' animal], 'jpg');
...
% % % 
% % figure;
% % imagesc(t_passive,[],buf2')
% % colormap( ap.colormap('PWG'));
% % clim(0.001 .* [-1, 1]);
% 
% 
ap.imscroll(image_all{9}(:,:,:,:),use_t)
axis image off
ap.wf_draw('ccf','black');
clim(0.005.*[-1,1]);
colormap(ap.colormap('PWG'));
