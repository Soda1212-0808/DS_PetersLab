...
animals={'DS030','DS031','DS029','DS005'};
% animals={'DS005'};
0
...
for curr_animal_idx=1:length(animals)
    ...
    recordings=recordings( 1:find(strcmp({recordings.day},wf_tform.day(end))));
    ...
    if     exist ([Path 'task\' animal '_task.mat' ])==2
        ...
        wf_px_task_kernels_encode = cell(size(recordings));
        ...
        stim2move=cell(length(recordings),1);
        stim2move_correct=cell(length(recordings),1);
        wf_px_task_all_type_id= cell(length(recordings),1);
        wf_px_task_all_reward_id= cell(length(recordings),1);
        wf_px_task_all= cell(length(recordings),1);
        wf_px_task_kernels_all= cell(length(recordings),1);
        wf_px_task_all_itimove=cell(length(recordings),1);
        tasktype=cell(length(recordings),1);
    ...
    if ~(file_length==length(recordings))
        for curr_recording =file_length+1:length(recordings)
            ...
            workflow_type_name{curr_recording}=recordings(curr_recording).workflow{index_real};
            % workflow_type_name_merge{curr_recording}=workflow_name_map(recordings(curr_recording).workflow{index_real});
...
if contains(bonsai_workflow,'earphone')& mean(audio_onsets-stimOn_times)>0.1
