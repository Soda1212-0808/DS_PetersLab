...
% 找到最长数组的长度
% 创建 NaN 填充的矩阵，右对齐填充数据
aligned_stage0_mean =cellfun(@(x) median( cell2mat(cellfun(@(v) [v;nan( max(cellfun(@length, x)) - length(v),1) ], x, 'UniformOutput', false)'), 2,'omitmissing'),stim2move_med_stage0,'UniformOutput',false);
aligned_stage0_error =cellfun(@(x) std( cell2mat(cellfun(@(v) [v;nan( max(cellfun(@length, x)) - length(v),1) ], x, 'UniformOutput', false)'), 0,2,'omitmissing')/sqrt(length(x)),stim2move_med_stage0,'UniformOutput',false);
...
xticks([3,  5+0.5*length(aligned_stage1_mean{curr_fig}) ,...
  5+length(aligned_stage1_mean{curr_fig})+0.5*length(aligned_stage2_mean{curr_fig})  ] ); % 设置 y 轴的刻度位置（2代表naive stage中间位置，8代表stage1中间位置）
...
ap.errorfill(6:5+length(aligned_stage1_perform{curr_fig}),...
    aligned_stage1_perform{curr_fig}, aligned_stage1_perform_error{curr_fig},line_color{curr_fig},0.1,0.5);
ap.errorfill(6+length(aligned_stage1_perform{curr_fig}):...
   5+length(aligned_stage1_perform{curr_fig})+length(aligned_stage2_perform{curr_fig}),...
    aligned_stage2_perform{curr_fig}, aligned_stage2_perform_error{curr_fig},line_color{curr_fig},0.1,0.5);
...
%% First association day in stage1 % 2
