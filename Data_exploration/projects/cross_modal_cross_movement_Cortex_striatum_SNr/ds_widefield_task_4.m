...
for curr_animal_idx=1:length(animals)
    ...
    if ~(file_length==length(recordings2)&problem==0)
        for curr_recording =file_length:length(recordings2)
            ...
            elseif contains( workflow_type_name{curr_recording}, {'cross'})
                workflow_type(curr_recording) = 3;
            elseif contains( workflow_type_name{curr_recording}, {'stage1', 'stage2'}) && ...
                    (contains( workflow_type_name{curr_recording}, {'opacity', 'size_up', 'angle','Vcenter'}) || ...
            ...
            elseif workflow_type(curr_recording)==3
                        [~,~,curr_tasktype_0]=unique([trial_events.values.TaskType]);
                curr_tasktype_0=curr_tasktype_0';
            ...
            aligned_v_avg=   permute( feval(@(c) cat(1,c{:}) ,...
                arrayfun(@(id) nanmean(aligned_v_baselinesub(align_id==id,:,:),1), unique(align_id),'UniformOutput',false)),[3,2,1]);
            ...
            elseif workflow_type(curr_recording)==3
                stim_regressors = {histcounts(real_stimOn_times(curr_tasktype==1),wf_regressor_bins);...
                    histcounts(real_stimOn_times(curr_tasktype==2),wf_regressor_bins)};
                move_regressors = {histcounts(real_stim_move_time(curr_tasktype==1),wf_regressor_bins);...
                    histcounts(real_stim_move_time(curr_tasktype==2),wf_regressor_bins)};
                reward_regressors= {histcounts(real_reward_times(curr_tasktype(rewarded_trials(1:n_trials))==1),wf_regressor_bins);...
                    histcounts(real_reward_times(curr_tasktype(rewarded_trials(1:n_trials))==2),wf_regressor_bins)};
            ...
            while ~success
                try
                    ...
                    [stim_kernels,predicted_signals,explained_var] = ...
                        cellfun(@(y) ap.regresskernel(wf_V(1:n_components,:),y,-frame_shifts,lambda),...
                         stim_regressors ,'UniformOutput',false );
 % 
 % [stim_kernels,predicted_signals,explained_var] = ...
 ...
 %                        wf_t_only_task, stim_regressors ,'UniformOutput',false );
            ...
            while ~success
                try
                    ...
                    [move_kernels,predicted_signals,explained_var] = ...
                        cellfun(@(y) ap.regresskernel(wf_V(1:n_components,:),y,-frame_shifts,lambda),...
                         move_regressors ,'UniformOutput',false );
            ...
            while ~success
                try
                    ...
                    [reward_kernels,predicted_signals,explained_var] = ...
                        cellfun(@(y) ap.regresskernel(wf_V(1:n_components,:),y,-frame_shifts,lambda),...
                         reward_regressors ,'UniformOutput',false );
