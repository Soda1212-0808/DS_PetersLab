...
% stimOn_times=stimOn_times(stim_to_lastmove>0)
...
% title(name_id(curr_passive))
 % ap.wf_draw('ccf','k');
  % ap.wf_draw('probe_insertion',animal);

end

images=ds.colormap_overlay(kernels_px_all_max(:,:,1),kernels_px_all_max(:,:,2),'B','R',[],0.0003);
figure;
image(images)
clim(0.0002*[-1 1]); axis image off;
 ap.wf_draw('ccf','k');
...
wf_regressor_bins = [wf_t;wf_t(end)+1/wf_framerate];
iti_move_regressors=histcounts(iti_move_time{2},wf_regressor_bins);
