...
% On this plot, we can draw a dot for each unit at it's CCF location:
% scatter3(template_ccf(:,1),template_ccf(:,3),template_ccf(:,2),10,'k','filled');

idx = find(ismember(probe_histology.acronym,brain_region));

% idx(probe_histology.probe_shank(idx)==1)
region_dist=arrayfun(@(probe) probe_histology.tip_distance(idx(probe_histology.probe_shank(idx)==probe),:),1:4,'UniformOutput',false);

temp_id=find(feval(@(x) sum( cat(2,x{:}),2), arrayfun(@(probe) template_tipdist>region_dist{probe}(2) & template_tipdist<region_dist{probe}(1)...
    &  template_shanks==probe,1:4,'UniformOutput',false)));

ap.ccf_outline_3d([],{brain_region});
scatter3(template_ccf(temp_id,1),template_ccf(temp_id,3),template_ccf(temp_id,2),10,'k','filled');

%%
ds.process_ephys

resp_idx=find(ephys_data.response_p{2}>0.99);

idx_all=intersect(temp_id,resp_idx)
ap.ccf_outline_3d([],{brain_region});


scatter3(template_ccf(idx_all,1)+ 5 * (rand(size(idx_all)) - 0.5),...
    template_ccf(idx_all,3)+  5* (rand(size(idx_all)) - 0.5),template_ccf(idx_all,2),10,'k','filled');

figure;
nexttile
plot(permute(nanmean(ephys_data.psth(temp_id,:,:),1),[2,3,1]))
...
curr_area=neuron_id.area{count_id};
%
...
spks= double(binned_spikes(curr_neurons_id,:));
...
task_name=task_name_all (unique(behavior.tasktype+1))
mainfig=figure('Name',[ animal ',' rec_day ',' curr_area])


main_tile=tiledlayout(mainfig,1,5,'TileIndexing','columnmajor')
sgtitle([ animal ',' rec_day ',' curr_area])
nexttile
imagesc(sortIdx')
colormap(ap.colormap(['BWR']));

for curr_state=1:length( unique(behavior.tasktype))
    t_stage = tiledlayout(main_tile,3,1,"TileSpacing","tight",'Padding','tight');
    t_stage.Layout.Tile =  curr_state+1;
    nexttile(t_stage)
    imagesc(unit_psth_t,[],psth_all_selected(sortIdx',:,curr_state))
    clim([-3 3])
    xlim([-0.1 0.5])
    xline(0)
    colormap(ap.colormap(['BWR']));
    ...
    nexttile
    plot_mean=nanmean(psth_all_selected(:,:,curr_state),1);
    plot_error=std(psth_all_selected(:,:,curr_state),0,1,"omitmissing")./sqrt(size(psth_all_selected,1));
    xline(0)

    ap.errorfill(unit_psth_t,plot_mean,plot_error,[0 0 0])
    xlim([-0.1 0.5])
    ylim(fr_scale)
    ylabel('FR')
    set(gca,'Color','none')

    align_velocity= arrayfun(@(a) behavior.velocity(behavior.tasktype==a & behavior.outcome==1,:) ,unique(behavior.tasktype),'uni',false);
    align_velocity_mean=cellfun(@(x)  nanmean(x,1),align_velocity,'UniformOutput',false);
    align_velocity_error=cellfun(@(x) std(x,0,1,"omitmissing")./sqrt(size(x,1)),align_velocity,'UniformOutput',false  );

    nexttile
    ap.errorfill(surround_time_points,align_velocity_mean{curr_state},align_velocity_error{curr_state},[0 0 0])
    xlim([-0.1 0.5])
    ylim([-2000 2000])
    ylabel('velocity')
    xline(0)
...
mua_corr1 = corrcoef([binned_spikes_depth ]');


figure;
imagesc(mua_corr1);

imagesc(depth_corr_bin_centers,depth_corr_bin_centers,mua_corr1);
axis image;
clim([-1,1].*0.5);
colormap(ap.colormap('BWR'))
...
elseif contains(bonsai_workflow,'stim_wheel')
    ...
    if ~isfield(trial_events.values,'TaskType')
        ...
        %     TaskTypes(vertcat(trial_events.values(1:n_trials).Outcome)==0)+0.5;
        % trial_sort_idx=buff*2;
...
% B=[unit_psth(:,:,1) unit_psth(:,:,2) unit_psth(:,:,3) unit_psth(:,:,4)]
figure;
imagesc(B)
hold on
...
[coeff, score, latent, tsquared, explained, mu]=pca(B(1:232,:));
figure;
...
ap.plot_unit_depthrate(spike_templates,spike_templates,template_depths,probe_areas,ax_h)
