...
% animals = {'DS007','DS010','AP019','AP021','DS011','DS001','AP018','AP020'};n1_name='visual position';n2_name='audio volume'; index_group=[1 1 1 1 1 1 0 0 0 ]';
...
matches=cellfun(@(x) unique(x, 'stable'),data1.all_animal_workflow_name(find(index_group)),'UniformOutput',false);

n1_data = cellfun(@(x,y) x(strcmp(y,n1_name)),data1.all_animal_stim2move_med(find(index_group)),data1.all_animal_workflow_name(find(index_group)),'UniformOutput',false);
n1_data_null = cellfun(@(x,y) x(strcmp(y,n1_name)),data1.all_animal_stim2move_med_null(find(index_group)),data1.all_animal_workflow_name(find(index_group)),'UniformOutput',false);
n1_reindx=cellfun(@(x,y) x(strcmp(y,n1_name)),data1.all_animal_react_index(find(index_group)),data1.all_animal_workflow_name(find(index_group)),'UniformOutput',false);

n1_move_fraction=cellfun(@(x,y) x(strcmp(y,n1_name),:),data1.all_animal_workflow_day_frac_move(find(index_group)),data1.all_animal_workflow_name(find(index_group)),'UniformOutput',false);
n2_data = cellfun(@(x,y) x(strcmp(y,n2_name)),data1.all_animal_stim2move_med(find(index_group)),data1.all_animal_workflow_name(find(index_group)),'UniformOutput',false);
n2_data_null = cellfun(@(x,y) x(strcmp(y,n2_name)),data1.all_animal_stim2move_med_null(find(index_group)),data1.all_animal_workflow_name(find(index_group)),'UniformOutput',false);
n2_reindx=cellfun(@(x,y) x(strcmp(y,n2_name)),data1.all_animal_react_index(find(index_group)),data1.all_animal_workflow_name(find(index_group)),'UniformOutput',false);
n2_move_fraction=cellfun(@(x,y) x(strcmp(y,n2_name),:),data1.all_animal_workflow_day_frac_move(find(index_group)),data1.all_animal_workflow_name(find(index_group)),'UniformOutput',false);
...
n1_data_null= cellfun(@(x,y) x(y==1),n1_data_null,n1_reindx1,'UniformOutput',false);
n1_move_fraction= cellfun(@(x,y) x(y==1,:),n1_move_fraction,n1_reindx1,'UniformOutput',false);
...
n1_filled_pre_null = cellfun(@(x) padarray(x', [0 max_len_n1-numel(x)], NaN, 'pre'), n1_data_null, 'UniformOutput', false);
n1_filled_pre_move_fraction = cellfun(@(x) padarray(x,  max_len_n1-size(x,1), NaN, 'pre'), n1_move_fraction, 'UniformOutput', false);

% 对n2操作
n2_reindx2=  cellfun(@(A) [zeros(1, find( A > 0, 1)-1), ones(1, numel(A) - find( A > 0, 1) + 1)],n2_reindx, 'UniformOutput', false);
n2_data= cellfun(@(x,y) x(y==1),n2_data,n2_reindx2,'UniformOutput',false);
n2_data_null= cellfun(@(x,y) x(y==1),n2_data_null,n2_reindx2,'UniformOutput',false);
n2_move_fraction= cellfun(@(x,y) x(y==1,:),n2_move_fraction,n2_reindx2,'UniformOutput',false);

% 1. 获取最长向量的长度
max_len_n2 = max(cellfun(@numel, n2_data));
% 2. 使用 NaN 填充较短的向量
n2_filled_post = cellfun(@(x) padarray(x', [0 max_len_n2-numel(x)], NaN, 'post'), n2_data, 'UniformOutput', false);
n2_filled_pre = cellfun(@(x) padarray(x', [0 max_len_n2-numel(x)], NaN, 'pre'), n2_data, 'UniformOutput', false);
n2_filled_post_null = cellfun(@(x) padarray(x', [0 max_len_n2-numel(x)], NaN, 'post'), n2_data_null, 'UniformOutput', false);
n2_filled_post_move_fraction=cellfun(@(x) padarray(x, max_len_n2-size(x,1), NaN, 'post'), n2_move_fraction, 'UniformOutput', false);

n1_n2=[cell2mat(n1_filled_pre)  cell2mat(n2_filled_post)]';
n1_n2_null=[cell2mat(n1_filled_pre_null)  cell2mat(n2_filled_post_null)]';
n1_n2_move_fraction=cellfun(@(x,y) [x; y], n1_filled_pre_move_fraction,n2_filled_post_move_fraction,'UniformOutput',false);

move_fraction_mean=nanmean(cat(3,n1_n2_move_fraction{:}),3);
react_index=cell2mat(cellfun(@(x) (max(x(:,surround_time_points>0&surround_time_points<1),[],2)-max(x(:,surround_time_points>-2&surround_time_points<-1),[],2))./...
    (max(x(:,surround_time_points>0&surround_time_points<1),[],2)+max(x(:,surround_time_points>-2&surround_time_points<-1),[],2)),...
    n1_n2_move_fraction,'UniformOutput',false)');

%%
figure('Position',[50 50 800 250],'Name',[ 'reaction time in ' n1_name '_to_' n2_name])
tiledlayout(1, 3, 'TileSpacing', 'compact', 'Padding', 'compact');
nexttile
hold on
% plot(n1_n2,'Color', [0.8 0.8 1]);
% plot(n1_n2_null,'Color', [0.8 0.8 0.8]);
plot1=nanmean(n1_n2,2);
sem_s2m_v=std(n1_n2,0,2,'omitmissing')/sqrt(size(n1_n2,5));
% h1= plot(plot1,'Color', [0 0 1],'LineWidth',2);
% plot2=nanmean(n1_n2_null,2);
% h2= plot(plot2,'Color', [0 0 0],'LineWidth',2);

ap.errorfill(1:length(nanmean(n1_n2,2)),plot1, sem_s2m_v,[0 0 1],0.1,0.5);
xticks([(length(n1_filled_pre{1})-4)/2,  (length(n1_filled_pre{1})-2) , length(n1_filled_pre{1})+(length(n2_filled_pre{1})+1)/2 ] ); % 设置 y 轴的刻度位置（2代表naive stage中间位置，8代表stage1中间位置）
xticklabels({'pre learn','post learn 1','learn 2'}); % 设置对应的标签
...
uistack(bg3, 'bottom');
sgtitle([n1_name ' to ' n2_name])


nexttile
imagesc(surround_time_points,[],move_fraction_mean)
clim([0 1])
yline(length(n1_filled_pre{1})-4.5)
yline(length(n1_filled_pre{1})+0.5)
yticks([(length(n1_filled_pre{1})-4)/2,  (length(n1_filled_pre{1})-2) , length(n1_filled_pre{1})+(length(n2_filled_pre{1})+1)/2 ] ); % 设置 y 轴的刻度位置（2代表naive stage中间位置，8代表stage1中间位置）
yticklabels({'pre learn','post learn 1','learn 2'}); % 设置对应的标签
colormap(gca,ap.colormap('WK'));
xlabel('time (s)')

nexttile
hold on
ap.errorfill(1:length(nanmean(react_index,2)),nanmean(react_index,2), std(react_index,0,2,'omitmissing')/sqrt(size(react_index,2)),[0 0 0],0.1,0.5);
xline(length(n1_filled_pre{1})-4.5)
xline(length(n1_filled_pre{1})+0.5)
xticks([(length(n1_filled_pre{1})-4)/2,  (length(n1_filled_pre{1})-2) , length(n1_filled_pre{1})+(length(n2_filled_pre{1})+1)/2 ] ); % 设置 y 轴的刻度位置（2代表naive stage中间位置，8代表stage1中间位置）
xticklabels({'pre learn','post learn 1','learn 2'}); % 设置对应的标签
xlim([1 length(nanmean(react_index,2))])
ylim([0 0.5])
ylabel('index')

saveas(gcf,[Path 'figures\summary\behavior\reaction time in ' n1_name '_to_' n2_name], 'jpg');
...
sem_s2m_v=std(cat(2,s2m_v1{:}),0,2)/sqrt(length(all_animal_stim2move_mean(animals_group_mixed==1)))

mean_s2m_a=mean(cat(2,s2m_a1{:}),2)
sem_s2m_a=std(cat(2,s2m_a1{:}),0,2)/sqrt(length(all_animal_stim2move_mean(animals_group_mixed==1)))
