...
% alignment_path = fullfile(plab.locations.server_path,'Lab','mousecam_alignment');
% alignment_path_animal = fullfile(alignment_path,'animal_alignment');
alignment_path_animal='C:\Users\dsong\Documents\MATLAB\Da_Song\Data_analysis\mice\process\mousecam_alignment';

alignment_filename = fullfile(alignment_path_animal,sprintf('mousecam_alignment_%s.mat',animal));
...
switch align_type
   ...
   % Select control points for selected image
        ...
        if strcmp(user_input,'s')
            mc_tform = struct;
            mc_tform.animal = animal;
            mc_tform.day = reshape(day,[],1);
    ...
    case 'new_animal'
        ...
        confirm_save = strcmp(input( ...
            ...
            's'),'y');
        if confirm_save
            mc_tform.animal_tform = im_tform.T;
            mc_tform.master_ref_size = ref_size;
            save(alignment_filename,'mc_tform');
    ...
    otherwise
        ...
        curr_day_idx = strcmp(day,mc_tform.day);
        ...
        switch align_type
            case 'day_only'
                ...
                curr_tform = mc_tform.day_tform{curr_day_idx};
                ref_size = mc_tform.ref_size;
            case 'animal_only'
                % Align animal only (used if already day-aligned)
                curr_tform = mc_tform.animal_tform;
                ref_size = mc_tform.master_ref_size;
            otherwise
                % Apply both day and animal alignments (if available)
                curr_day_tform = mc_tform.day_tform{curr_day_idx};
                if isfield(mc_tform,'animal_tform') && ~isempty(mc_tform.animal_tform)
                    curr_animal_tform = mc_tform.animal_tform;
