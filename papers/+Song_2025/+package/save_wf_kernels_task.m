...
wf_task_kernel_stim_aligned=cell(2,1);
...
for curr_group=1:2
    ...
    all_data_video_stim=cell(length(animals{curr_group}),1);
    ...
    for curr_animal=1:length(animals{curr_group})
        ...
        image_all_stim(idx)=cellfun(@(x)  x{1},raw_data_task.wf_px_task_kernels(idx),'UniformOutput',false);
        image_all_move(idx)=cellfun(@(x)  x{2},raw_data_task.wf_px_task_kernels(idx),'UniformOutput',false);
    ...
    wf_task_kernel_stim_each_mice{curr_group}=all_data_video_stim;
    ...
    for t = 1:numel(names)
        ...
        if numel(out) >= 7 && ~isempty(out{7})
            ch_v = rules(t,1);
            ch_a = rules(t,2);
            val_v = cellfun(@(x) cellfun(@(n) n(:,:,ch_v), x, 'UniformOutput', false), ...
                out{7}, 'UniformOutput', false);
            val_a = cellfun(@(x) cellfun(@(n) n(:,:,ch_a), x, 'UniformOutput', false), ...
                out{7}, 'UniformOutput', false);
    ...
    wf_task_kernel_stim_aligned{curr_group}    = wf.stim;
...
%     1:size(wk,1), 'UniformOutput', false), wf_task_kernel_stim_aligned,'UniformOutput',false);
% 
% 
% 
% % wk=wf_task_kernel_stim_aligned{1}
% 
...
% wf_task_kernels_stim_across_day= cellfun(@(x)    feval(@(C) cat(4, C{:}),cellfun(@(a)   cat(3,a{:}),x,'UniformOutput',false))   ,temp_1,'UniformOutput',false);
...
for idx = 1:numel(names)
    ...
    out = cellfun(@(x) feval(@(C) cat(4, C{:}), cellfun(@(a) cat(3, a{:}), x, 'UniformOutput', false)), ...
                  temp_1, 'UniformOutput', false);

    % 存到结果结构里
    wf_task_kernels_across_day.(names{idx}) = out;
end

% 可选：如果你想把结果解包成单独变量（取消下面注释行）
wf_task_kernels_stim_across_day   = wf_task_kernels_across_day.stim;
