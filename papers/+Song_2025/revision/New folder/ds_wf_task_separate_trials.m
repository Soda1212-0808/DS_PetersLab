...
for curr_animal_idx=1:length(animals)
    ...
    if     exist ([Path 'task\separate_trials\' animal '_task.mat' ])==2
        load([Path 'task\separate_trials\' animal '_task.mat' ])
        ...
        if isempty (n_buffer)
            file_length=  length(wf_px_task_kernels);
            problem=0;
        ...
        wf_px_task_raw= cell(size(recordings2));
    ...
    if ~(file_length==length(recordings2)&problem==0)
        for curr_recording =file_length+1:length(recordings2)
            ...
            if workflow_type(curr_recording)==1|workflow_type(curr_recording)==2
                ...
                temp_q = quantile(real_stim_to_move, [1/3, 2/3]);  % 三分位点
                stim_to_move_idx = zeros(size(real_stim_to_move));
                stim_to_move_idx(real_stim_to_move <= temp_q(1)) = 1;
                stim_to_move_idx(real_stim_to_move > temp_q(1) & real_stim_to_move <= temp_q(2)) = 2;
                stim_to_move_idx(real_stim_to_move > temp_q(2)) = 3;


                stim_regressors= arrayfun(@(a)  histcounts(real_stimOn_times(stim_to_move_idx==a),wf_regressor_bins),...
                ...
                wf_t_only_task=arrayfun(@(a) interp1([gap_1(stim_to_move_idx==a);gap_2(stim_to_move_idx==a)],...
            ...
            elseif workflow_type(curr_recording)==3
                ...
                curr_tasktype=[trial_events.values(1:n_trials).TaskType];
                all_types=unique(curr_tasktype);

                temp_q=  cellfun(@(x) quantile(x, [1/3, 2/3]) ,...
                    arrayfun(@(type) real_stim_to_move(curr_tasktype==type),unique(curr_tasktype),'uni',false),'UniformOutput',false);

                stim_to_move_idx = zeros(size(real_stim_to_move));


                for curr_type=1:2
                    type=all_types(curr_type);

                    stim_to_move_idx(real_stim_to_move <= temp_q{curr_type}(1)& curr_tasktype'==type) = 1+3*(type);
                    stim_to_move_idx(real_stim_to_move > temp_q{curr_type}(1) & real_stim_to_move <= temp_q{curr_type}(2)& curr_tasktype'==type) = 2+3*(type);
                    stim_to_move_idx(real_stim_to_move > temp_q{curr_type}(2)& curr_tasktype'==type) = 3+3*(type);
                end
            ...
            baseline_times =real_stimOn_times(rewarded_trials);
            peri_event_t = reshape(real_stimOn_times(rewarded_trials),[],1) + reshape(t_task,1,[]);
            baseline_event_t = reshape(baseline_times,[],1) + reshape(baseline_t,1,[]);
            aligned_v = reshape(interp1(wf_t,wf_V',peri_event_t,'previous'), ...
                length(real_stimOn_times(rewarded_trials)),length(t_task),[]);
            ...
            aligned_v_avg = permute(splitapply(@nanmean,aligned_v_baselinesub,stim_to_move_idx(rewarded_trials)),[3,2,1]);
            
            wf_px_task_raw{curr_recording}=aligned_v_avg;
            ...
            wf_px_task_kernels{curr_recording}=cat(3,stim_kernels{:});
    ...
    save([Path 'task\separate_trials\' animal '_task.mat' ],'workflow_type','workflow_type_name',...
        'workflow_type_name_merge',...
        'wf_px_task_kernels','wf_px_task_raw','img_size','workflow_day', '-v7.3')
